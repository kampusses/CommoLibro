...
qualcosa da muovere. Il nostro primo codice avrà quindi un obiettivo molto più modesto dell'intero videogioco: fare
comparire sullo schermo la nostra automobile.
...
\subsection{Prima prepariamo lo schermo}
Il nostro primo listato comincia senza occuparsi ancora dell$\text{\textgreek{’}}$automobile. Prima di mostrare
qualsiasi cosa, vogliamo impostare la condizione iniziale dello schermo sul quale lavoreremo. Le prime istruzioni
operative sono queste:
...
L$\text{\textgreek{’}}$istruzione PRINT non ci è nuova. Molti anni fa il professor Rossi Brunori
l$\text{\textgreek{’}}$aveva utilizzata per farmi vedere comparire sullo schermo il primo CIAO e avevamo imparato ad
associarla, almeno inizialmente, all$\text{\textgreek{’}}$idea di mostrare qualcosa sul monitor.

Questa volta, però, dopo PRINT non troviamo una parola racchiusa tra virgolette, compare invece qualcosa di nuovo:

chr\$(147)

CHR\$ è una funzione del BASIC che riceve un numero compreso tra 0 e 255 e restituisce il carattere associato a quel
codice. Il valore ottenuto può quindi essere utilizzato da PRINT, esattamente come qualsiasi altra stringa. Questo non
significa necessariamente che sullo schermo debba comparire un simbolo.

Il Commodore 64 utilizza infatti una propria codifica dei caratteri, oggi comunemente conosciuta come \textbf{PETSCII},
nella quale accanto a lettere, numeri, segni di punteggiatura e simboli grafici troviamo anche numerosi \textbf{codici
di controllo}. Alcuni spostano il cursore, altri modificano il colore del testo, attivano o disattivano il reverse,
cambiano il set di caratteri oppure eseguono operazioni sullo schermo. Il numero 147 appartiene proprio a questa
seconda categoria.

print chr\$(147)

non stampa quindi il simbolo corrispondente al numero 147, ma invia al sistema il codice \textbf{CLR/HOME}, che pulisce
lo schermo e riporta il cursore nella posizione iniziale. 

È per questo che la nostra prima istruzione operativa produce l$\text{\textgreek{’}}$impressione che il programma inizi
da uno schermo nuovo. Potremmo anche ometterla. Il programma continuerebbe a funzionare, ma ciò che vedremmo
dipenderebbe da quello che si trovava sul monitor prima del RUN: vecchi comandi, messaggi, eventuali prove effettuate
in precedenza. Preferiamo invece che sia il programma stesso, per quanto possibile, a costruire lo stato dal quale
vuole partire.

\subsubsection{Esperimento – Che cosa nasconde CHR\$?}
Prima di continuare con il nostro gioco vale la pena fermarsi per qualche minuto e lasciare che sia il Commodore a
mostrarci che cosa significhi realmente CHR\$.

In modalità diretta proviamo:
...
Questa volta il codice corrisponde a un carattere visibile.

Proviamo invece:

print chr\$(18);{\textquotedbl}COMMODORE 64{\textquotedbl}

Il codice 18 attiva la modalità reverse e il testo successivo viene visualizzato con i caratteri invertiti. Possiamo
disattivarla con:

print chr\$(146)

Oppure possiamo provare:

print chr\$(30);{\textquotedbl}VERDE{\textquotedbl}

e osservare che il codice 30 modifica il colore del testo in verde. Anche colori, movimento del cursore e reverse
possono infatti essere controllati attraverso appositi codici CHR\$. 

Possiamo persino tornare al punto da cui siamo partiti:

print chr\$(147)

e cancellare in un istante tutte le prove appena eseguite.

La tabella PETSCII comprende tutti i valori da 0 a 255 e contiene, oltre ai codici di controllo, lettere, numeri,
punteggiatura e numerosi simboli grafici. Troveremo la raccolta integrale nell$\text{\textgreek{’}}$appendice B
dedicata, dove potrà essere utilizzata come strumento di consultazione.

\subsubsection{Laboratorio – Esploriamo PETSCII}
Gli esperimenti precedenti utilizzavano valori che conoscevamo già. Possiamo adesso lasciare per qualche minuto il
controllo al lettore.

Scriviamo questo piccolo programma:
...
Lo scopo non è ancora studiare nel dettaglio FOR e NEXT; li incontreremo tra poco nel vero programma dello sprite e sarà
allora che ne comprenderemo bene il funzionamento. Per il momento possiamo considerarli semplicemente come un modo per
chiedere al Commodore di ripetere le righe comprese fra 20 e 40 utilizzando l$\text{\textgreek{’}}$incremento di valori
successivi di C da 32 a 126.

Eseguiamo il programma e osserviamo il risultato.

Accanto a ogni numero vedremo il carattere che CHR\$ gli associa. Possiamo quindi modificare
l$\text{\textgreek{’}}$intervallo, provare valori differenti e confrontare ciò che appare sullo schermo.

Conviene però non partire subito da 0 TO 255. In quell$\text{\textgreek{’}}$intervallo sono presenti numerosi codici di
controllo: alcuni sposterebbero il cursore, cambierebbero colore o modalità di visualizzazione e CHR\$(147)
cancellerebbe addirittura il risultato dell$\text{\textgreek{’}}$esperimento. È molto più istruttivo esplorare prima un
intervallo prevalentemente stampabile e poi provare individualmente i codici di controllo che ci incuriosiscono.

Quando avremo finito, sappiamo già come ripulire tutto, sia la memoria (con l$\text{\textgreek{’}}$istruzione NEW, sia
lo schermo):
...
e possiamo tornare al nostro programma.
